...
class Graph:
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        """
        a, b = sorted((str(source), str(target)))   # sorted returns a list, but we unpack to a,b
        if a == b:                                  # skip self-loops
            return
        edge = (a, b)                               # <-- make it a TUPLE
    ...
    def max_degree_nodes(self) -> dict:
        ...
        or {'a': 22, 'b': 22}
        """
        # Start every node degree at 0
        degrees = {nid: 0 for nid, _ in self.nodes}
        ...
        return {nid: deg for nid, deg in degrees.items() if deg == max_degree}
    ...
    # Do not modify
    def write_edges_file(self, path="Q1/edges.csv")->None:
        """
    ...
    # Do not modify
    def write_nodes_file(self, path="Q1/nodes.csv")->None:
        """
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list[int]=None) -> list:
        ...
        headers = {
            ...
            "Authorization": "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI0YjI5YjM3ZDZkOGJlODNhMzU1MTM1NTgzOGIzZWM5YyIsIm5iZiI6MTc1NjAwMzIyOS44OTUsInN1YiI6IjY4YWE3YjlkYzFkNzg2YTg2NDU0N2VmNiIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.fPmgkz7uhvhdO2kxO3FFY4ubqj5zEtWK_qjtOj-w5AI"
        }

        response = requests.get(url, headers=headers)
        cast = response.json().get("cast", [])

        # 1) Keep only items with a valid integer 'order'
        cast = [c for c in cast if isinstance(c.get("order"), int)]

        # 2) Exclude by person 'id' (not cast_id)
        if exclude_ids:
            banned = set(int(x) for x in exclude_ids)
            cast = [c for c in cast if int(c.get("id", -1)) not in banned]

        # 3) Sort by billing order (deterministic)
        cast.sort(key=lambda d: d["order"])

        # 4) Apply limit semantics: keep ONLY those with order < limit (no backfilling)
        if limit is not None:
            cast = [c for c in cast if c["order"] < limit]
    ...
    def get_movie_credits_for_person(self, person_id: str, start_date: str = None, end_date: str = None) -> list:
        ...
        cast_dicts = response['cast']
        # print(cast_dicts)
        cast_credits = [c for c in cast_dicts if(c['release_date'] <= end_date) and (c['release_date'] >= start_date)]
        # print(cast_credits)
        keys = ("id", "title", "release_date")    # keys to keep
        cast_credits_ = [{k: d[k] for k in keys if k in d} for d in cast_credits]
        # print(cast_credits_)
        return cast_credits_
...
SEED_PERSON_ID = "2975"
YEAR = "1999"
START_1999 = f"{YEAR}-01-01"
END_1999   = f"{YEAR}-12-31"

def build_coactor_graph_1999(api: TMDBAPIUtils, g: Graph) -> Graph:
    # Seed node (name hard-coded per assignment; commas sanitized in add_node)

    # Helper: add cast members for a movie (top-5 billed) and connect to a given actor_id
    def add_movie_top5(actor_id: str, movie_id: str, exclude_self_id: str):
        cast = api.get_movie_cast(movie_id, limit=5, exclude_ids=[int(exclude_self_id)])
        for c in cast:
            co_id = str(c.get("id"))
            co_name = c.get("name", "")
            if not co_id or not co_name:
                continue
            g.add_node(co_id, co_name)
            g.add_edge(actor_id, co_id)

    # 1) BASE GRAPH: all 1999 movies for seed; add top-5 cast edges to seed
    base_new_nodes = set()
    seed_credits_1999 = api.get_movie_credits_for_person(SEED_PERSON_ID, START_1999, END_1999)
    for credit in seed_credits_1999:
        add_movie_top5(SEED_PERSON_ID, credit["id"], exclude_self_id=SEED_PERSON_ID)
        # collect nodes added this step (approximate via current g.nodes)
        for nid, _ in g.nodes:
            if nid != SEED_PERSON_ID:
                base_new_nodes.add(nid)

    # 2) EXPAND TWICE:
    prev_added = list(base_new_nodes)
    for _ in range(2):
        current_added = set()
        for actor_id in prev_added:
            # This actor's 1999 movies
            credits_1999 = api.get_movie_credits_for_person(actor_id, START_1999, END_1999)
            for credit in credits_1999: 
                add_movie_top5(actor_id, credit["id"], exclude_self_id=actor_id)
            # Accumulate newly seen nodes from graph at this point
            for nid, _ in g.nodes:
                if nid != SEED_PERSON_ID and nid not in prev_added:
                    current_added.add(nid)
        prev_added = list(current_added)
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='4b29b37d6d8be83a3551355838b3ec9c')

    # call functions or place code here to build graph (graph building code not graded)
    # Suggestion: code should contain steps outlined above in BUILD CO-ACTOR NETWORK

    graph = build_coactor_graph_1999(tmdb_api_utils, graph)

    print(f"Nodes: {graph.total_nodes()}  Edges: {graph.total_edges()}")
    graph.write_edges_file()
    graph.write_nodes_file()

    # tmdb_api = TMDBAPIUtils('4b29b37d6d8be83a3551355838b3ec9c')

    # tmdb_api.get_movie_cast(movie_id=24, limit=6 , exclude_ids=[139, 140])
    # tmdb_api.get_movie_credits_for_person(person_id=16, start_date=None, end_date='2025-05-05')
